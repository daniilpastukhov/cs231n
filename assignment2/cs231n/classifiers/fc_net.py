...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers + 1):
            self.params[f'W{i}'] = weight_scale * np.random.randn(hidden_dims[i - 1], hidden_dims[i])
            self.params[f'b{i}'] = np.zeros((hidden_dims[i]))
            if normalization == 'batchnorm':
                self.params[f'gamma{i}'] = np.array([1])
                self.params[f'beta{i}'] = np.array([0])
    ...
    def loss(self, X, y=None):
        ...
        for i in reversed(range(1, self.num_layers)):
            dx, dw, db = affine_relu_backward(dx, cache_history[i])
